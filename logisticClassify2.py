...
## METHODS ################################################################
    ...
    def plotBoundary(self,X,Y):
        ...
        x2b =  np.array([(-self.theta[0] - self.theta[1]*x1b[0])/self.theta[2], (-self.theta[0] - self.theta[1]*x1b[1])/self.theta[2]])     
        A = Y== self.classes[0]; # and plot it:
        plt.plot(X[A,0],X[A,1],'b.',X[np.logical_not(A),0],X[np.logical_not(A),1],'r.',x1b,x2b,'k-');
        plt.axis(ax);
        plt.draw();
