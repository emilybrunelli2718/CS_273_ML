...
## MUTATORS ####################################################################
    ...
    # def setActivation(self, method, sig=None, sig0=None): 
        ...
        if method == 'logistic':
            ...
            #self.Sig = lambda z: twod(z/(z+np.exp(-z)))
            #self.dSig = lambda z: twod(z/(z+np.exp(-z)) + (1 / (1 + np.exp(-z)))*(1-z/(z+np.exp(-z))))
            #self.sig_0 = self.sig
            #self.d_sig_0 = self.d_sig
        else:
...
## MUTATORS ####################################################################
    ...
    def setActivation(self, method, sig=None, sig0=None): 
        ...
        if method == 'logistic':
            ...
            self.sig = lambda z: twod(z/(z+np.exp(-z)))
            self.d_sig = z/(z+np.exp(-z)) + (1 / (1 + np.exp(-z)))*(1-z/(z+np.exp(-z)))
            self.sig_0 = self.sig
            self.d_sig_0 = self.d_sig
        else:
